...
@section('contenido')
    ...
    <form method="post" action="{{route('Pub.edit',$publicacion->id)}}" name="EditarPublicacion">
        @csrf
        ...
        <div class="field">
            ...
            <textarea aria-label="Descripcion." name="descripcion" class="textarea is-primary"
                      placeholder="">{{$publicacion->tituloSNFormato}}</textarea>
        </div>
    </form>
    <div class="field">
    ...
    <button type="button" class="button is-primary is-medium is-fullwidth" id="publicar">{{__('publication.post')}}</button>
    <script>
        var toolbarOptions = [
        ...
        var contenido = new Quill('#publicacion', optionsPub);
        ...
        btnPublicar.onclick=function() {
            titulobd = titul.getContents();
            otrotitulo=titul.getText();
            publicacionbd = contenido.getContents();
            if (titulobd.ops[0].insert !== "\n" && publicacionbd.ops[0].insert !== "\n") {
                ...
                document.EditarPublicacion.submit();
            } else {
